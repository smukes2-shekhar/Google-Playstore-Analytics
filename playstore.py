...
from sklearn.metrics import mean_absolute_error # 0.3 error uncertainity
from sklearn.impute import SimpleImputer
from sklearn.ensemble import RandomForestRegressor
from sklearn.model_selection import train_test_split

# reading the dataset from kaggle
df = pd.read_csv('C:/Users/13126/Downloads/googleplaystore.csv')

# cleaning the data set in regards to size column
df['Size'] = df['Size'].map(lambda x: x.rstrip('M'))
df['Size'] = df['Size'].map(lambda x: str(round((float(x.rstrip('k'))/1024), 1)) if x[-1]=='k' else x)
df['Size'] = df['Size'].map(lambda x: np.nan if x.startswith('Varies') else x)

# M = millions, varies = NA, k = turning into Millions to get a common size parameter

# cleanig the data set in regards to Installation column
df['Installs'] = df['Installs'].map(lambda x: x.rstrip('+'))
df['Installs'] = df['Installs'].map(lambda x: ''.join(x.split(',')))

# cleaning the data set for the price column (stripping the $ sign)
...
# 10427 row was containing empty values so it was removed
df.drop(df.index[10472], inplace=True)
#sorting the raw data due to errors, to eliminate random sorting
...
#using the NA columns with means and dropping entries after the possible tranformations

# Change datatype to numeric for analysis
